...
export default function SimpleTabs() {
  ...
  //  const [Rerror, seterror] = useState<error[]>([]);
    ...
    const onSubmit: SubmitHandler<User> = (data) => {
      ...
      const usersCollectionRef = collection(db, 'Verror');
...
// 所得
  ...
  const UserComponent: React.FC<{ user: { code: string } }> = ({ user }) => {
    return <div className="user-name">{user.code}</div>;
  };
  ...
  const deleteUser = async (subtitle: string) => {
    const userCollectionRef = collection(db, 'Verror');
    const q = query(userCollectionRef, where('subtitle', '==', subtitle));
    const querySnapshot = await getDocs(q);
    querySnapshot.forEach(async (document) => {
      const userDocumentRef = doc(db, 'Verror', document.id);
    ...
    <div className={styles.box1}>
        ...
        <div className={styles.box_main}>
          <div key={user.name}>
                  ...
                  <summary className={styles.names}>
                    ...
                    <button className={styles.sakuzyo} key={user.subtitle} onClick={() => deleteUser(user.subtitle)}>削除</button>
                  </summary>
      ...
      <TabPanel value={value} index={1}>
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
                <label>エラー文</label><br/>
                <textarea className={styles.input_title} defaultValue="エラー" {...register('error')} />
            </div>
            <div>
                <label>詳細</label><br/>
